...
&nbsp;&nbsp;&nbsp;Figure 2 illustrates the current translation of the geologic layers to hydrostratigraphic units within the ORMGP conceptual model. This north-south cross section extends from Lake Simcoe in the north across the Oak Ridges Moraine (ORM) to Lake Ontario in the south.  Interpreted groundwater flow directions are shown as blue arrows.  The main aquifer units are the ORM sediments, ORM-channel (C2), Thorncliffe Formation channels (C1), and sands of the Thorncliffe Formation and the Scarborough Formation. The main aquitard units are the glaciolacustrine/surficial diamict deposits (e.g., Halton Till), the Newmarket Till, and Lower Sediment silt, clay and diamict (e.g., Sunnybrook Drift). 

![*Figure 2*](https://raw.githubusercontent.com/OWRC/GeoLayers/main/images/Fig29-CONCEP_MODEL_Hydrostrat-Portrait-REG-20230405.png)
...
- If the top of Newmarket Till elevation is below Bedrock,  then set top of Newmarket Till elevation to Bedrock elevation;
- If top of Thorncliffe Formation elevation is above the top of NewmarketTill , then set top of Thorncliffe Formation elevation to top of Newmarket Till elevation;
...
The order that these post-processing rules are processed is important, as each surface check relied on the preceeding checks and constrants. The approach is not a top-down correction. For example,constraining the Halton till is one of the last checks performed.
